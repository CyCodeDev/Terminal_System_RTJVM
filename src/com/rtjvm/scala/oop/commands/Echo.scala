...
class Echo(args: Array[String]) extends Command {
  ...
  def getRootAfterEcho(currentDirectory: Directory, path: List[String], contents: String, append: Boolean): Directory = {
    if (path.isEmpty) currentDirectory
      ...
      val newNexDirectory = getRootAfterEcho(nextDirectory, path.tail, contents, append)
      if(newNexDirectory == nextDirectory) currentDirectory
      else currentDirectory.replaceEntry(path.head, newNexDirectory)
    }
